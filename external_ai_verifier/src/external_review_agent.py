...
Agente principal de revisÃ£o externa - ponto de entrada do mÃ³dulo
"""
...
class ExternalReviewAgent:
    """Agente de revisÃ£o externa - orquestrador principal do mÃ³dulo"""

    def __init__(self, config_path: Optional[str] = None):
        """
        Inicializa o agente de revisÃ£o externa

        Args:
            config_path (Optional[str]): Caminho para arquivo de configuraÃ§Ã£o
        """
        # Inicializar logger da instÃ¢ncia
        ...
        # --- NOVO: ConfiguraÃ§Ã£o para Rate Limiting do LLM ---
        # Calcula o delay necessÃ¡rio para 9 requisiÃ§Ãµes por minuto (60 segundos)
        # Delay entre requisiÃ§Ãµes = 60s / 9 = 6.67s (aproximadamente)
        # Adicionamos um pequeno buffer para garantir o limite.
        self.llm_call_delay = self.config.get('rate_limiting', {}).get('llm_call_delay_seconds', 10)
        self.last_llm_call_time = 0.0 # Para rastrear o tempo da Ãºltima chamada LLM
        # ----------------------------------------------------
        ...
        logger.info(f"âœ… External Review Agent inicializado com sucesso")
        logger.info(f"ðŸ”§ ConfiguraÃ§Ãµes carregadas: {len(self.config)} seÃ§Ãµes")
        # Log do delay configurado
        logger.info(f"â³ Rate Limiting LLM: {self.llm_call_delay:.2f}s entre chamadas")
            ...
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    if config_path.endswith('.yaml') or config_path.endswith('.yml'):
                        ...
                        config = json.load(f)
                logger.info(f"âœ… ConfiguraÃ§Ã£o carregada: {config_path}")
                return config
            else:
                logger.warning(f"âš ï¸ Arquivo de configuraÃ§Ã£o nÃ£o encontrado: {config_path}")
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        # Adiciona a seÃ§Ã£o rate_limiting com delay padrÃ£o
        return {
            'thresholds': {
                'approval': 0.75,
                'rejection': 0.35,
            ...
            'rules': [],
            'rate_limiting': {
                'llm_call_delay_seconds': 6.7 # PadrÃ£o: ~9 chamadas por minuto
            }
        }
    ...
    def _enforce_llm_rate_limit(self):
        ...
        if time_since_last_call < self.llm_call_delay:
            ...
            logger.debug(f"Rate limiting: aguardando {sleep_time:.2f}s antes da prÃ³xima chamada LLM")
    ...
    def process_item(self, item_data: Dict[str, Any], massive_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Processa um item individual atravÃ©s de todas as anÃ¡lises com validaÃ§Ã£o aprimorada

        Args:
            item_data (Dict[str, Any]): Dados do item para anÃ¡lise
            massive_data (Optional[Dict[str, Any]]): Contexto adicional

        Returns:
            Dict[str, Any]: Resultado completo da anÃ¡lise
        """
        start_time = datetime.now()

        try:
            item_id = item_data.get('id', f'item_{self.stats["total_processed"]}')
            logger.info(f"ðŸ” Iniciando anÃ¡lise do item: {item_id}")

            # ValidaÃ§Ã£o prÃ©via do item
            validation_result = self._validate_item_data(item_data)
            if not validation_result['valid']:
                logger.warning(f"âš ï¸ Item invÃ¡lido: {validation_result['reason']}")
            ...
            if not text_content or len(text_content.strip()) < 5:
                logger.warning("âš ï¸ Item com conteÃºdo textual insuficiente")
            ...
            logger.debug("Executando anÃ¡lise de sentimento...")
            ...
            logger.debug("Executando detecÃ§Ã£o de viÃ©s/desinformaÃ§Ã£o...")
            ...
            if should_use_llm:
                logger.debug("Executando anÃ¡lise LLM...")
                # --- Aplica rate limit ANTES da chamada LLM ---
                ...
                analysis_result['llm_reasoning_analysis'] = {
                    'llm_confidence': 0.5,
                    'llm_recommendation': 'NÃƒO_EXECUTADO',
                    'analysis_reasoning': 'LLM nÃ£o necessÃ¡rio para este item'
                }

            # Step 4: Contextual Analysis
            logger.debug("Executando anÃ¡lise contextual...")
            ...
            logger.debug("Aplicando regras de negÃ³cio...")
            ...
            logger.info(f"âœ… Item processado: {final_decision['status']} (confianÃ§a: {final_decision['final_confidence']:.3f})")
    ...
    def _validate_item_data(self, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(item_data, dict):
            return {'valid': False, 'reason': 'Item deve ser um dicionÃ¡rio'}
        
        if not item_data:
            return {'valid': False, 'reason': 'Item vazio'}
        
        # Verificar se tem pelo menos um campo de conteÃºdo
        content_fields = ['content', 'text', 'title', 'description', 'summary', 'body']
        has_content = any(field in item_data and item_data[field] for field in content_fields)
        
        if not has_content:
            return {'valid': False, 'reason': 'Item nÃ£o possui conteÃºdo textual vÃ¡lido'}
        
        return {'valid': True, 'reason': 'Item vÃ¡lido'}

    def _extract_text_content(self, item_data: Dict[str, Any]) -> str:
        """Extrai conteÃºdo textual do item com priorizaÃ§Ã£o"""
        # Campos priorizados por importÃ¢ncia
    ...
    def _should_use_llm_analysis(self, sentiment_result: Dict[str, Any], bias_result: Dict[str, Any]) -> bool:
        """Determina se deve usar anÃ¡lise LLM"""
    ...
    def _create_llm_context(self, analysis_result: Dict[str, Any], massive_data: Optional[Dict[str, Any]]) -> str:
        """Cria contexto para anÃ¡lise LLM"""
        context_parts = []
        ...
        if bias.get('overall_risk', 0) > 0.3:
            context_parts.append(f"Risco de viÃ©s detectado: {bias.get('overall_risk', 0):.2f}")

        # Add any available external context
        if massive_data:
            if 'topic' in massive_data:
                context_parts.append(f"TÃ³pico: {massive_data['topic']}")

        return ' | '.join(context_parts) if context_parts else ""

    def _make_final_decision(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        """Toma decisÃ£o final baseada em todas as anÃ¡lises"""
        try:
            ...
            if rule_decision.get('status') in ['approved', 'rejected']:
                ...
                if self.confidence_thresholds.should_approve(final_confidence):
                    status = 'approved'
                    reason = 'Aprovado com base na anÃ¡lise combinada'
                elif self.confidence_thresholds.should_reject(final_confidence):
                    status = 'rejected'
                    reason = 'Rejeitado com base na anÃ¡lise combinada'
                else:
                    # Default to rejection for ambiguous cases (safer)
                    status = 'rejected'
                    reason = 'Rejeitado por ambiguidade - polÃ­tica de seguranÃ§a'
            ...
            decision = {
                ...
                'decision_factors': {
                    ...
                    'llm_recommendation': llm.get('llm_recommendation', 'NÃƒO_EXECUTADO'),
            ...
            logger.error(f"Erro na decisÃ£o final: {e}")
            return {
                'status': 'rejected',
    ...
    def _create_validation_error_result(self, item_data: Dict[str, Any], reason: str) -> Dict[str, Any]:
        """Cria resultado para item que falhou na validaÃ§Ã£o"""
        return {
            ...
            'ai_review': {
                'status': 'rejected',
                'reason': f'Erro de validaÃ§Ã£o: {reason}',
                'final_confidence': 0.0,
                'error': True,
    ...
    def _create_insufficient_content_result(self, item_data: Dict[str, Any]) -> Dict[str, Any]:
        """Cria resultado para item com conteÃºdo insuficiente"""
        return {
            'item_id': item_data.get('id', 'sem_id'),
            'original_item': item_data,
            'processing_timestamp': datetime.now().isoformat(),
            'ai_review': {
                'status': 'rejected',
                'reason': 'ConteÃºdo textual insuficiente para anÃ¡lise',
                'final_confidence': 0.0,
                'error': False,
    ...
    def find_consolidacao_file(self, session_id: str) -> Optional[str]:
        """Busca automaticamente o arquivo de consolidaÃ§Ã£o da etapa 1 para a sessÃ£o especificada"""
        try:
            ...
            for base_path in base_paths:
                if os.path.exists(base_path):
                    # Busca por arquivos de consolidação com múltiplos padrões
                    patterns = [
                    ...
                    for pattern in patterns:
                        ...
                        if files:
                            # Filtra arquivos que contenham session_id no nome ou conteúdo
                    ...
                    if files:
                        ...
                        self.logger.info(f"âœ… Arquivo de consolidaÃ§Ã£o encontrado: {latest_file}")
                        return latest_file

            # Se nÃ£o encontrou, busca em todo o projeto
            search_patterns = [
            ...
            for pattern in search_patterns:
                ...
                if files:
                    # Filtra por sessÃ£o se possÃ­vel
                    session_files = [f for f in files if session_id in f]
                    if session_files:
                        latest_file = max(session_files, key=os.path.getmtime)
                        self.logger.info(f"âœ… Arquivo de consolidaÃ§Ã£o encontrado (busca recursiva): {latest_file}")
                        return latest_file
                    else:
                        # Pega o mais recente se nÃ£o conseguir filtrar por sessÃ£o
                        latest_file = max(files, key=os.path.getmtime)
                        self.logger.info(f"âš ï¸ Usando arquivo mais recente: {latest_file}")
                        return latest_file

            self.logger.warning(f"âŒ Nenhum arquivo de consolidaÃ§Ã£o encontrado para sessÃ£o {session_id}")
            return None

        except Exception as e:
            self.logger.error(f"âŒ Erro ao buscar arquivo de consolidaÃ§Ã£o: {e}")
            return None

    def load_consolidacao_data(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Carrega dados do arquivo de consolidaÃ§Ã£o da etapa 1"""
        try:
            ...
            self.logger.info(f"ðŸ“„ Dados de consolidaÃ§Ã£o carregados: {len(data.get('data', {}).get('dados_web', []))} itens web")
            return data

        except Exception as e:
            self.logger.error(f"âŒ Erro ao carregar dados de consolidaÃ§Ã£o: {e}")
            return None

    def convert_consolidacao_to_analysis_format(self, consolidacao_data: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        """Converte dados de consolidaÃ§Ã£o para formato de anÃ¡lise do External AI Verifier"""
        try:
            ...
            self.logger.info(f"âœ… ConversÃ£o concluÃ­da: {len(items)} itens preparados para anÃ¡lise")
            ...
            self.logger.error(f"âŒ Erro ao converter dados de consolidaÃ§Ã£o: {e}")
            return {'items': [], 'context': {}}

    def analyze_session_consolidacao(self, session_id: str) -> Dict[str, Any]:
        """Analisa automaticamente os dados de consolidaÃ§Ã£o de uma sessÃ£o"""
        try:
            self.logger.info(f"ðŸ” Iniciando anÃ¡lise da consolidaÃ§Ã£o para sessÃ£o: {session_id}")

            # Carrega dados de consolidaÃ§Ã£o
            ...
            # Adiciona informaÃ§Ãµes da sessÃ£o ao resultado
            ...
            self.logger.error(f"âŒ Erro na anÃ¡lise da sessÃ£o {session_id}: {e}")
            return {
                'success': False,
                'error': str(e),
                'session_id': session_id,
    ...
    def process_batch(self, items: List[Dict[str, Any]], massive_data: Optional[Dict[str, Any]] = None, 
        ...
        """
        logger.info(f"ðŸš€ Iniciando processamento em lote: {len(items)} itens, lotes de {batch_size}")
        ...
        for i in range(0, len(items), batch_size):
            ...
            logger.info(f"ðŸ“¦ Processando lote {batch_num}/{total_batches} ({len(batch)} itens)")
            ...
            logger.info(f"âœ… Lote {batch_num} concluÃ­do: {len([r for r in batch_results if r.get('ai_review', {}).get('status') == 'approved'])} aprovados")
    ...
    async def process_batch_async(self, items: List[Dict[str, Any]], massive_data: Optional[Dict[str, Any]] = None,
        ...
        """
        Processa itens de forma assÃ­ncrona para melhor performance
        
        Args:
            items: Lista de itens para processar
            massive_data: Contexto adicional
            max_concurrent: MÃ¡ximo de processamentos simultÃ¢neos
            
        Returns:
            Dict com resultados do processamento assÃ­ncrono
        """
        logger.info(f"âš¡ Iniciando processamento assÃ­ncrono: {len(items)} itens, mÃ¡x {max_concurrent} simultÃ¢neos")
        
        semaphore = asyncio.Semaphore(max_concurrent)
        
        async def process_single_item(item):
            async with semaphore:
                # Como process_item nÃ£o Ã© async, executar em thread
        ...
        # Processar resultados e exceÃ§Ãµes
        valid_results = []
        approved_items = []
        rejected_items = []
        errors = []
        ...
        for i, result in enumerate(all_results):
            if isinstance(result, Exception):
                logger.error(f"âŒ Erro no item {i}: {result}")
    ...
    def analyze_content_batch(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not items:
                return {
                    'success': False,
                    'error': 'Nenhum item fornecido para anÃ¡lise',
                    'timestamp': datetime.now().isoformat()
                }

            self.logger.info(f"ðŸ” Iniciando anÃ¡lise de {len(items)} itens")

            results = []
            total_items = len(items)

            for idx, item in enumerate(items):
                self.logger.info(f"ðŸ“Š Analisando item {idx + 1}/{total_items}: {item.get('id', 'N/A')}")

                try:
                    result = self.process_item(item, context) # Use process_item directly
                    results.append(result)

                    # Pequeno delay entre anÃ¡lises para nÃ£o sobrecarregar outros recursos (opcional)
                    # if idx < total_items - 1:
                    #     import time
                    #     time.sleep(0.1) # 100ms entre itens

                except Exception as e:
                    self.logger.error(f"âŒ Erro ao analisar item {item.get('id', 'N/A')}: {e}")
                    results.append({
            ...
            self.logger.error(f"âŒ Erro na anÃ¡lise em lote: {e}")
            return {
                'success': False,
                'error': str(e),
                'timestamp': datetime.now().isoformat()
    ...
    def _generate_batch_statistics(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Gera estatÃ­sticas para uma anÃ¡lise em lote."""
...
def run_external_review(input_data: Dict[str, Any], config_path: Optional[str] = None) -> Dict[str, Any]:
    """
    FunÃ§Ã£o principal de entrada para o mÃ³dulo externo

    Args:
        input_data (Dict[str, Any]): Dados de entrada contendo itens para anÃ¡lise
        config_path (Optional[str]): Caminho para arquivo de configuraÃ§Ã£o

    Returns:
        Dict[str, Any]: Resultados da anÃ¡lise e itens processados
    """
    try:
        logger.info("ðŸš€ Iniciando External AI Verifier...")
        ...
        if not items:
            logger.warning("Nenhum item fornecido para anÃ¡lise")
            return {
                'items': [],
        ...
        logger.info(f"âœ… Processamento concluÃ­do: {len(approved_items)} aprovados, {len(rejected_items)} rejeitados")

        return final_result

    except Exception as e:
        logger.error(f"Erro crÃ­tico no External AI Verifier: {e}")
        return {
            'items': [],
...
# InstÃ¢ncia global do External AI Verifier (usando ExternalReviewAgent)
